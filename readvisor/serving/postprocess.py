...
"""Config/data loaders and output post-processing for the serving app.

Handles two concerns:

* simple file loaders (config JSON, company gazetteer, company-label mapping);
* post-processing of generated hypotheses — masking named entities, filling the
  masks from an input review's metadata, and re-ranking the n-best list.
"""
...
def load_config(config_file_path: str) -> SimpleNamespace:
    ...
    """
    with open(config_file_path, encoding="utf8") as cfg_file:
        # Parse JSON into an object with attributes corresponding to dict keys.
        cfg = json.loads(cfg_file.read(), object_hook=lambda d: SimpleNamespace(**d))
    logger.info("loaded config file from %s", config_file_path)
    return cfg
...
def load_company_data_from_text(text_file: str) -> Set[str]:
    ...
    logger.info("loaded company data from text file %s", text_file)
    return companies
...
def load_company_label_mapping(tsv_file: str) -> Dict:
    ...
    logger.info("loaded company-label mapping data from tsv file %s", tsv_file)
...
def parse_greetingtext(greetingtype: str, lang: str) -> str:
    ...
    if greetingtype == "informally":
        ...
        logger.warning("Could not get greeing for greetingtype value %s", greetingtype)
        return ""
...
def parse_goodbyetext(string: str, lang: str) -> str:
    ...
    if pref_format:
        if lang == "de":
            ...
            logger.warning("Could not parse salutation for language %s", lang)
            return ""
...
def fill_masks(text: str, meta: Dict) -> str:
    ...
    if meta.get("greetings") is not None:
        ...
        logger.warning("`greetings` attribute in metadata is N/A")
    ...
    if meta.get("salutations") is not None:
        ...
        logger.warning("`salutations` attribute in metadata is N/A")
...
def postprocess_object(
    ...
    src_tokens = {tok for tok in src_doc if not tok.is_stop and not tok.is_punct}
    ...
    for doc in nlp.pipe(map(add_greeting_salutation_masks, hyp_texts)):
        ...
        # FIXME: src_doc.similarity(doc) is called without SBERT vectors loaded
        # (serving loads spaCy via load_spacy_pipe with sbert_model=None), so
        # spaCy falls back to (near-empty) tensor vectors and this similarity
        # term is effectively noise in the re-ranking. Left as-is (untestable here).
    ...
    response_object["responses"] = apply_reordering(
        processed_hyps, model_scores, overlap_scores, sts_scores
    )
...
def compute_word_overlap(src_tokens: str, hyp_doc: str) -> float:
    ...
    """
    # FIXME: `not tok.is_stop or not tok.is_punct` is almost always True (only
    # false for a token that is BOTH a stopword AND punctuation), so this filter
    # is a near no-op — it likely should be `and`. Left as-is (untestable here).
    hyp_tokens = {tok for tok in hyp_doc if not tok.is_stop or not tok.is_punct}
    if len(src_tokens):
