...
def trim_embedding_matrix_of_pretrained_model(
    ...
    """Trim a pretrained mBART embedding matrix down to ``reduce_to_vocab``.

    Loads ``base_model`` and its tokenizer, keeps only the sentencepiece pieces
    listed in ``reduce_to_vocab`` (plus special/language tokens), rewrites the
    embedding matrix and ``final_logits_bias`` accordingly, and saves the reduced
    model and tokenizer to ``save_model_to``. Returns the ``(model, tokenizer)``.
    """
    ...
    logger.info("original_vocab_size %s", original_vocab_size)
    logger.info("model_size %s", model_size)
    # trim embed matrix
    if reduce_to_vocab is not None:
        with open(reduce_to_vocab) as f:
            ...
            logger.info("len keep_pieces: %s", len(keep_pieces))
            ...
            logger.info("new_vocab_size %s", new_vocab_size)
            ...
            logger.info("added_vocab_length %s", added_vocab_length)
            base_vocab_length_original = original_vocab_size - added_vocab_length
            logger.info("base_vocab_length_original %s", base_vocab_length_original)
            base_vocab_length_new = len(keep_pieces) + num_special_tokens
            logger.info("base_vocab_length_new %s", base_vocab_length_new)
            ...
            for embed_iter, spm_iter in zip(range(4,base_vocab_length_original), range(3,base_vocab_length_original-1)): # full vocab size with (!) the added tokens, 250027 |
                ...
                if new_embed_iter > base_vocab_length_new:
                    logger.info(
                        "ran out of space at position %s in new matrix with vocab size %s",
                        new_embed_iter, base_vocab_length_new,
                    )
                    exit(0)
            ...
            logger.info("total count matched %s", count)
            logger.info(
                "len vocabs to keep %s + special tokens %s",
                len(keep_pieces.keys()), num_special_tokens,
            )
            logger.info("new vocab size %s", new_vocab_size)
...
def read_in_symbols(infile: str) -> list:
    """Read whitespace-separated symbols from ``infile`` into a sorted list."""
    symbols = set()
...
def main() -> None:
    """Parse CLI args, trim the mBART embedding matrix, and optionally verify it."""
    ...
    logger.info("loaded tokenizer with len %s", len(tokenizer.sp_model))
    ...
    if user_special_tokens:
        ...
        logger.info("added %s special tokens to tokenizer", num_added_toks)
        model.resize_token_embeddings(len(tokenizer))

        logger.info("saving tokenizer with new tags")
        tokenizer.save_pretrained(args.save_model_to)
        logger.info("saving model with new tags")
        model.save_pretrained(args.save_model_to)

    logger.info(tokenizer.special_tokens_map)
    logger.info(tokenizer.id_to_lang_code)
    logger.info(tokenizer.lang_code_to_id)
    ...
    if args.verbose > 0:
        ...
        logger.info("decoder start ids %s", decoder_start_token_ids)
        ...
        for translation in translations:
            if not len(translation.strip()):
                logger.warning('[!] Looks like something went wrong - translation is empty.')
            else:
